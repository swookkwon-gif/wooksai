...
# 하위 호환: 기존 scripts/state.json이 존재하면 우선 사용
_LEGACY_STATE = os.path.join(os.path.dirname(__file__), '..', 'state.json')
if os.path.exists(_LEGACY_STATE) and not os.path.exists(STATE_FILE):
    import shutil
    shutil.copy2(_LEGACY_STATE, STATE_FILE)
    print(f"  [마이그레이션] state.json을 state/ 디렉토리로 복사했습니다.")
