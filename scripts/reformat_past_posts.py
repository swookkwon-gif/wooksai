...
4. **[주석 보존 및 양식 변경]**: 
   - 기존 본문에 있던 주석 번호(예: [1], [2] 혹은 <sup>[[1]]...</sup>)는 마크다운 표준 주석 문법인 `[^1]` 형태로 변환하세요.
   - 포스트 가장 하단의 레퍼런스(참고문헌) 목록은 1번부터 번호 순서대로 정렬하고, 각 항목을 `[^1]: [기사 제목](URL)` 형태로 작성하세요.
   - **중요**: 만약 참고문헌의 정확한 원본 URL을 찾을 수 없다면, 가짜 URL을 생성하지 말고 `[^1]: 기사 제목` 형식으로 링크 없이 텍스트만 표기하세요. 자신이 아는 올바른 URL이 있다면 반드시 링크를 걸어주세요.
